...
AnaTtresSB::AnaTtresSB(const std::string &filename, bool electron, bool boosted, bool semiB, bool overlap, std::vector<std::string> &systList)
  : Analysis(filename, systList), m_electron(electron), m_boosted(boosted), m_semiB(semiB), m_overlap(overlap),
  ...
  m_hSvc.m_tree->Branch("mtt_sb",    &_tree_mtt_sb);
...
//    m_hSvc.create1DP(name, "; #Delta R; Events", 31, 0.2, 0.5, 0., 1.);
  ...
  if (m_semiB) {
    m_hSvc.create1D("largeJetsbM", "; large jet mass sb [GeV] ; Events", 30, 0, 300);
    m_hSvc.create1DVar("largeJetsbPt", "; large jet p_{T} [GeV] ; Events", varBinN2, varBin2);
    m_hSvc.create1DVar("goodJetPt", "; good Jet p_{T} [GeV] ; Events", varBinN1, varBin1);
    m_hSvc.create1D("LargeJetSBEta", "; large jet #eta ; Events", 20, -2., 2.);
    m_hSvc.create1D("LargeJetSBPhi", "; large jet #phi [rd] ; Events", 32, -3.2, 3.2);
    m_hSvc.create1D("LargeJetSBSd12", "; large jet #sqrt{d_{12}} [GeV] ; Events", 20, 0, 200);
    m_hSvc.create1D("mtt_res_sb", "; top-anti top mass resolution sb[GeV]; Events", 50, -1.5, 1.5);
    m_hSvc.create2D("mtt_res_sb_vs_ljet_dr_ajet", "; mtt_res_sb ; ljet_dr_ajet", 50, -1.5, 1.5, 50, 0, 5);
    m_hSvc.create2D("mtt_res_sb_vs_ljet_dr_lep", "; mtt_res_sb ; ljet_dr_lep", 50, -1.5, 1.5, 25, 0, 5);
    m_hSvc.create1D("topmass_sb", "; Top M ; Events", 50, 0, 300);
    m_hSvc.create1DVar("mtt_sb", "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
  }

  m_hSvc.create1D("lepCharge", "; lepton charge ; Events", 5, -2, 2);
  ...
  m_hSvc.create1DVar("trueMtt", "; mass of the top-antitop system [GeV]; Events", varBinN5, varBin5);
    m_hSvc.create1D("mtt_res", "; top-anti top mass resolution sb[GeV]; Events", 50, -1.5, 1.5);
...
AnaTtresSB::~AnaTtresSB() {
}

void AnaTtresSB::run(const Event &evt, double weight, const std::string &s) {
  // check channel
  //
  char name[200];

//if(!m_boosted && m_semiB && !m_overlap) std::cout<<"I'm a semi boosted event" << std::endl;
if(m_semiB && !m_boosted && !m_overlap && ((evt.passes("bejets_semiB") || evt.passes("bmujets_semiB")) && !(evt.passes("bejets") || evt.passes("bmujets")) && !(evt.passes("rejets") || evt.passes("rmujets")))) std::cout<< "I passed the semiB channel exclusively" << std::endl;
...
//if (m_semiB && (evt.passes("bejets_semiB") || evt.passes("bmujets_semiB"))) std::cout<< "I am Semi_Boosted!" << std::endl;
//  if (!m_boosted)	if(evt.jet().size()<4)	return;
//boost only
  if (m_boosted && !m_semiB && !m_overlap && (!(evt.passes("bejets") || evt.passes("bmujets")) || (evt.passes("bejets_semiB") || evt.passes("bmujets_semiB")))) return;
//semi only
  if (m_semiB && !m_boosted && !m_overlap && (!(evt.passes("bejets_semiB") || evt.passes("bmujets_semiB")) || (evt.passes("bejets") || evt.passes("bmujets")) || (evt.passes("rejets") || evt.passes("rmujets"))))return;
//resolved only
  if (!m_boosted && !m_semiB && !m_overlap && (!(evt.passes("rejets") || evt.passes("rmujets")) || ((evt.passes("bejets_semiB") || evt.passes("bmujets_semiB"))) || (evt.passes("bejets") || evt.passes("bmujets")))) return;
  //boosted semi boosted
  if (m_boosted && m_semiB && m_overlap && ((!(evt.passes("bejets_semiB") || evt.passes("bmujets_semiB")) || !(evt.passes("bejets") || evt.passes("bmujets"))) )) return;
  //resolved semi boosted
  if (!m_boosted && m_semiB && m_overlap && ((!(evt.passes("rejets") || evt.passes("rmujets")) || !(evt.passes("bejets_semiB") || evt.passes("bmujets_semiB"))) || (evt.passes("bejets") || evt.passes("bmujets")))) return;
  //nothing
  if (!(evt.passes("rejets") || evt.passes("rmujets")) && !(evt.passes("bejets_semiB") || evt.passes("bmujets_semiB")) && !(evt.passes("bejets") || evt.passes("bmujets"))) return;
  ...
  if (m_electron) {
    ...
    trig4 = evt.electron()[0].HLT_e120_lhloose();
        trig5 = evt.electron()[0].HLT_e26_lhtight_nod0_ivarloose();
    trig6 = evt.electron()[0].HLT_e60_lhmedium_nod0();
    trig7 = evt.electron()[0].HLT_e140_lhloose_nod0();
    bool trig_MC = trig1 || trig3 || trig4;
    bool trig_DT = trig2 || trig3 || trig4 || trig5 || trig6 || trig7;
  ...
  //
  TLorentzVector lgen;
//std::cout<< "leps is good" << std::endl;
...
*/
  ...
  h->h1D("lepCharge", "", suffix)->Fill(lepcharge, weight);
  ...
  float mtt = -1;
  float trueMTT = evt.MC_ttbar_beforeFSR().M();
  ...
  h->h1D("mtt_res", "", suffix)->Fill((mtt - trueMTT)/(trueMTT), weight);
    h->h1D("mtlep_boo", "", suffix)->Fill((sj+nu+l).M()*1e-3, weight);

    _tree_mtt = mtt*1e-3;
    _tree_weight = weight;
    _tree_cat = -1;
    if (evt.passes("bejetsWCR") && m_boosted && m_electron) _tree_cat = 0;
    if (evt.passes("bmujetsWCR") && m_boosted && !m_electron) _tree_cat = 1;
    _tree_syst = s;
    h->m_tree->Fill();
  ...
  h->h1D("mtt_res", "", suffix)->Fill((mtt - trueMTT)/(trueMTT), weight);
    ...
    h->m_tree->Fill();
  }  if(m_semiB && (evt.passes("bejets_semiB") || evt.passes("bmujets_semiB"))) {
std::cout<<"And I made it here!" << std::endl;
    size_t close_idx = 0;
    for (; close_idx < evt.jet().size(); ++close_idx)
      if (evt.jet()[close_idx].closeToLepton())
        break;
    ...
    h->h1D("closeJetPt", "", suffix)->Fill(sj.Perp()*1e-3, weight);
   size_t good_idx = 0;
   size_t good_idx_real = 0;
   float mindiff = 10000000000;
    size_t goodljetsb_idx = 0;
    for (; goodljetsb_idx < evt.largeJet().size(); ++goodljetsb_idx) {
      std::cout << "jet " << goodljetsb_idx << " pt = " << evt.largeJet()[goodljetsb_idx].mom().Perp() << ", good = " << evt.largeJet()[goodljetsb_idx].good() << std::endl;
      if (evt.largeJet()[goodljetsb_idx].good_sb())
         break;
        }
      const TLorentzVector &lj = evt.largeJet()[goodljetsb_idx].mom();
std::cout<<"and my large jet pt is: " << lj.Perp()*1e-3<<std::endl;
    for (; good_idx < evt.jet().size(); ++good_idx){
     if (std::fabs((evt.jet()[good_idx].mom() + lj).M() -173000) < mindiff){
          mindiff =std::fabs( (evt.jet()[good_idx].mom() + lj).M()*1e-3 -173);
         //std::cout << std::fabs( (evt.jet()[good_idx_real].mom() + lj).M()*1e-3 - 185)<<std::endl;
         good_idx_real = good_idx;
        }
      }
    const TLorentzVector &gj = evt.jet()[good_idx_real].mom();

std::cout<<"and my good jet pt is: " << gj.Perp()*1e-3<<std::endl;
    h->h1D("largeJetsbPt", "", suffix)->Fill(lj.Perp()*1e-3, weight);
    h->h1D("largeJetsbM", "", suffix)->Fill(lj.M()*1e-3, weight);
  ...
  float mtt_sb = -1;
  //  if(lj.M()>130*1e3)return; 
  mtt_sb = (lj+sj+gj+nu+l).M();
std::cout<< "And my mtt is : " << mtt_sb<< std::endl;
  h->h1D("mtt_sb", "", suffix)->Fill(mtt_sb*1e-3, weight);
//    h->h1D("lepCharge", "", suffix)->Fill(charge, weight);
  h->h1D("mtt_res_sb", "", suffix)->Fill((mtt_sb - trueMTT)/(trueMTT), weight);
  h->h1D("topmass_sb", "", s)->Fill((lj + gj).M()/1000, weight);
  _tree_mtt_sb = mtt_sb*1e-3;
  _tree_weight = weight;
  _tree_cat = -1;
  if (evt.passes("bejets_semiB") && m_boosted && m_electron) _tree_cat = 0;
  if (evt.passes("bmujets_semiB") && m_boosted && !m_electron) _tree_cat = 1;
  _tree_syst = s;
   h->m_tree->Fill();
 }
