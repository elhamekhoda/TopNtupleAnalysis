...
AnaTtresWJ::AnaTtresWJ(const std::string &filename, bool electron, bool boosted, std::vector<std::string> &systList)
...
AnaTtresWJ::~AnaTtresWJ() {
}

void AnaTtresWJ::run(const Event &evt, double weight, const std::string &s) {
  // check channel
  //
  char name[200];
  ...
  */
  ...
  if (m_boosted)
    if (!(evt.passes("bejetsWCR") || evt.passes("bmujetsWCR")))
      return;
...
*/
  ...
  if (m_boosted && (evt.passes("bejetsWCR") || evt.passes("bmujetsWCR"))) {
    ...
    for (; goodljet_idx < evt.largeJet().size(); ++goodljet_idx) {
      ...
      if (evt.largeJet()[goodljet_idx].notgood())
        break;
    }
